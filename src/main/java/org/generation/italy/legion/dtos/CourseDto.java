...
public class CourseDto {
    ...
    public static CourseDto fromEntity(Course c){
        return new CourseDto(c.getId(),c.getTitle(),c.getDescription(), c.getProgram(),
                c.getDuration(), c.isActive(),
                c.getCreatedAt().toString());
    }
    public static Iterable<CourseDto> fromEntityIterable(Iterable<Course> cr){
        return StreamSupport.stream(cr.spliterator(), false)
                .map(c -> CourseDto.fromEntity(c))
                .toList();
    }
}
